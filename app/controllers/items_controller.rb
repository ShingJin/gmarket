class ItemsController < ApplicationController
  ...
  def index
      @items = Item.where("show =?",true).where("recommend =?",false).search(params[:search],params[:bs]).order(sort_column + ' ' + sort_direction).paginate(:per_page =>50, :page => params[:page])  
